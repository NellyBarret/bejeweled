...
#include "SDL_image.h"
#include "SDL2/SDL_mixer.h"         //le son
...
//the names of the waves to play
...
{ 
    ...
    /*-----------CREATING THE SOUNDS EFFECTS FOR SWIPING THE JEWELS---------------*/
    ...
    //attributing the channels for sound effects
    ...
    if(music==NULL){
        cout<<"problem while loading the music"<<endl;
        cout<<Mix_GetError();
    }
    ...
    while (!quit)
    {
        ...
        if(event.type == SDL_MOUSEBUTTONDOWN)
        {
            ...
            if(event.button.x >= 150 && event.button.x <= 400 && event.button.y >= 200 && event.button.y <= 270)
            {
                ...
                //playing the music (the main theme)
                if(Mix_PlayMusic(music, -1)==-1)
                {
                cout<<"could not open the music"<<endl;
                cout<<Mix_GetError();
                exit(-1);
                }
            }
            ...
            if(end_game == false)
            {
                ...
                if(event.button.x >= margin_x && event.button.x <= margin_x+(8*width) && event.button.y >= margin_y && event.button.y <= margin_y+(8*width))
                {
                    ...
                    if(xA == 0 && yA == 0)
                    {
                        ...
                        Mix_PlayChannel(-1, _sample[1], 0); //sound effect
